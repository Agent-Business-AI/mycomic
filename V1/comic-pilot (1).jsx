...
// ============================================================================
// CONFIG — Change this to your FastAPI backend URL
// ============================================================================
const API_BASE = "http://localhost:8000";

// ============================================================================
// CONSTANTS — Based on reverse-engineered LlamaGen Internal API
// ============================================================================
const PRESETS = [
...
// ============================================================================
// API SERVICE — All calls to FastAPI backend
// ============================================================================
const api = {
  async health() {
  ...
  async uploadCharacterPhoto(file) {
    const presigned = await fetch(`${API_BASE}/api/characters/upload`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fileName: file.name, fileType: file.type || "image/jpeg" }),
    });
    if (!presigned.ok) throw new Error(`Presigned URL failed: ${presigned.status}`);
    const data = await presigned.json();
    const putUrl = data.url || data.signedUrl || data.presignedUrl;
    if (!putUrl) throw new Error("No presigned URL in response");
    // Step 2: Upload photo to S3 presigned URL (PUT)
    ...
    return data.fileUrl || `https://s.llamagen.ai/${data.key || ""}`.replace(/\/+$/, "") || putUrl;
  },
  ...
  async generate(characters, scenes, settings) {
    const payload = {
      ...
      settings: {
        ...
        font: settings.font, lang: settings.lang, size: settings.size,
...
// ============================================================================
// COMPONENT: Generation Progress & Results
// ============================================================================
...
export default function ComicPilot() {
  ...
  const [settings, setSettings] = useState({
    ...
    font: "cartoonist", lang: "en", size: "1024,1024", showCaption: true, needDialogs: true, bubble: "undefined",
  });
  ...
  const handleGenerate = async () => {
    try {
      ...
      // Step 1–3: Upload photos, create characters for those with photos
      ...
      for (let i = 0; i < updatedChars.length; i++) {
        ...
        if (c.imageFile && !c.s3Url) {
          ...
          try {
            ...
            // Step 3: Create character for comicRoles
            if (c.name && fileUrl) {
          ...
          // Already uploaded, create character if not done
          try {
            const created = await api.createCharacter({
              name: c.name,
              imageUrl: c.s3Url,
  ...
  // CRUD
  const addCharacter = () => { const c = { id: uid("ch"), name: "", gender: "male", age: 25, bodyType: "average", description: "", defaultEmotion: "Happy", defaultPose: "Standing with hands on hips", imageFile: null, imagePreview: null, s3Url: "" }; setCharacters((p) => [...p, c]); setExpandedChar(c.id); setActiveTab("characters"); };
      ...
      {backendOk === false && (
        <div className="max-w-4xl mx-auto px-4 mt-3">
          <div className="px-4 py-3 bg-red-500/10 border border-red-500/30 rounded-xl text-sm text-red-300">
            <strong>Backend not reachable</strong> at <code className="text-red-400">{API_BASE}</code>
            <pre className="mt-2 p-2 bg-gray-900 rounded-lg text-xs text-gray-300 overflow-x-auto font-mono">{`cd comic-pilot\npip install -r requirements.txt\nexport LLAMAGEN_SESSION_TOKEN="your-token"\nuvicorn comic_pilot_backend:app --reload --port 8000`}</pre>
          </div>
        </div>
      )}
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        ...
        {activeTab === "characters" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div><h2 className="text-lg font-bold">Characters</h2><p className="text-xs text-gray-500 mt-0.5">Each → <code className="text-amber-400">comicRoles[]</code> entry. Photos uploaded to S3 before generation.</p></div>
        ...
        {activeTab === "scenes" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div><h2 className="text-lg font-bold">Scenes</h2><p className="text-xs text-gray-500 mt-0.5">Each scene = 1 panel → <code className="text-amber-400">comicData[].panels[]</code></p></div>
        ...
        {activeTab === "settings" && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div><label className="block text-xs font-medium text-gray-400 mb-1">Story Prompt <span className="text-gray-500">(optional override)</span></label><textarea value={settings.storyPrompt} onChange={(e) => setSettings((s) => ({ ...s, storyPrompt: e.target.value }))} placeholder="Overall story..." rows={2} className="w-full px-3 py-2 bg-gray-900 border border-gray-600 rounded-lg text-white text-sm focus:border-amber-500 focus:outline-none resize-none" /></div>
            </div>
            <div><label className="block text-xs font-medium text-gray-400 mb-2">Style</label>
              <div className="grid grid-cols-3 gap-2">{PRESETS.map((p) => (
                <button key={p.id} onClick={() => setSettings((s) => ({ ...s, preset: p.id }))} className={`p-3 rounded-xl border text-left transition-all ${settings.preset === p.id ? "border-amber-500 bg-amber-500/10" : "border-gray-700 bg-gray-800/50 hover:border-gray-500"}`}>
                  <span className="text-2xl">{p.icon}</span><div className="text-sm font-medium mt-1">{p.label}</div><div className="text-xs text-gray-500 font-mono">{p.id}</div>
                </button>
              ))}</div>
            </div>
